...
model = Sequential()
model.add(Conv2D(32, (4, 4), strides=(4, 4) ,activation='relu',
...
policy = LinearAnnealedPolicy(EpsGreedyQPolicy(),
                        attr='eps',
                        value_max=1.0,
                        value_min=-1.0, # clip rewards just like in paper
                        value_test=0.0,
                        nb_steps=10**3)
...
dqn_agent = DQNAgent(model=model, nb_actions=num_steering_angles,
                     ...
                     enable_dueling_network=False, target_model_update=1e-2,
                     policy=policy, gamma=0.999)

dqn_agent.compile(Adam(lr=1e-5), metrics=['mse'])


dqn_agent.fit(env, callbacks=None, nb_steps=10**4, visualize=False, verbose=2)
